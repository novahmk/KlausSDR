...
function _sdrClean(value) {
    return (value === null || value === undefined) ? '' : String(value).replace(/\s+/g, ' ').trim();
}

function _sdrNormalizeKey(value) {
    return _sdrClean(value)
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .replace(/[^a-z0-9\s]/g, ' ')
...
function _sdrClampNumber(value, fallback, min, max) {
    const num = Number(value);
    return !Number.isFinite(num) ? fallback : Math.min(max, Math.max(min, Math.round(num)));
}
...
function getObjecaoContext(objecao) {
    return OBJECTION_CONTEXTS[normalizeObjecao(objecao)] || '';
}
...
function buildSdrContext(lead, analise, antiRep) {
    ...
    const faseCtx = FASE_CONTEXTS[fase] || FASE_CONTEXTS.fase_2_qualificacao;
    ...
    return [scoreInfo, faseCtx, objecCtx, objecoesAnteriores, _sdrClean(antiRep)]
        .filter(Boolean)
        .join('\n\n');
}
